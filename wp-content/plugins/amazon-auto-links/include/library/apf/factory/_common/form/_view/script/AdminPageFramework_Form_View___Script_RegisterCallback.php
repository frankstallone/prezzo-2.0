...
(function ( $ ) {
    ...
    $.fn.aAmazonAutoLinks_AdminPageFrameworkRepeatFieldCallbacks               = [];    // 3.8.8+
    ...
    $.fn.aAmazonAutoLinks_AdminPageFrameworkStoppedSortingSectionsCallbacks    = [];    // 3.8.0+
    
    /**
     * Gets triggered when the + (add) button of a repeatable field is pressed.
     */  
    $( document ).bind( 'amazon-auto-links_added_repeatable_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){          
        ...
        $.each( $.fn.aAmazonAutoLinks_AdminPageFrameworkAddRepeatableFieldCallbacks, function( iIndex, aCallback ) {
            ...
            if ( -1 === $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {                
                console.warn( 'Admin Page Framework (' + sFieldType + ' field type): The `added_repeatable_field` callback argument for the `registerAmazonAutoLinks_AdminPageFrameworkCallbacks` method is deprecated. Use `repeated_field` instead.' );
            }
     ...
     * 
     * Unlike the `amazon-auto-links_added_repeatable_field` event, this does not call the callback function
     ...
     */
    $( document ).bind( 'amazon-auto-links_repeated_field', function( oEvent, iCallType, oModelContainer ){
        ...
        $.each( $.fn.aAmazonAutoLinks_AdminPageFrameworkRepeatFieldCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
            var _aFieldTypes = aCallback[ 1 ]; // '_nested', 'inline_mixed' are built-in
            if ( -1 !== $.inArray( sFieldType, [ '_nested', 'inline_mixed' ] ) ) {
                return true;    // continue
     ...
     * @since       3.8.0
     */
    $( document ).bind( 'amazon-auto-links_stopped_sorting_sections', function( oEvent ){  

        var _oThisNode = jQuery( oEvent.target );
        $.each( $.fn.aAmazonAutoLinks_AdminPageFrameworkStoppedSortingSectionsCallbacks, function( iIndex, aCallback ) {
            var _hfCallback  = aCallback[ 0 ];
     ...
     */
    /* $( document ).bind( 'amazon-auto-links_removed_field', function( oEvent, sFieldType, sID, iCallType, iSectionIndex, iFieldIndex ){
        var _oThisNode = jQuery( oEvent.target );
     ...
     * @since    3.2.0 
     */
    $( document ).bind( 'amazon-auto-links_saved_widget', function( event, oWidget ){
    ...
    $.fn.registerAmazonAutoLinks_AdminPageFrameworkCallbacks = function( oCallbacks, aFieldTypeSlugs ) {
        ...
        $.fn.aAmazonAutoLinks_AdminPageFrameworkRepeatFieldCallbacks.push(  // 3.8.8+
        ...
        // 3.8.0
        $.fn.aAmazonAutoLinks_AdminPageFrameworkStoppedSortingSectionsCallbacks.push( 
