...
/**
 * A class that filters out items that are not eligible for FBA (Fulfillment by Amazon).
 *
 * @since   3.10.0
 */
class AmazonAutoLinks_UnitOutput__ProductFilter_ByFBA extends AmazonAutoLinks_UnitOutput__ProductFilter_Base {
    ...
    protected function _shouldProceed() {
        ...
        return ( boolean ) $this->_oUnitOutput->oUnitOption->get( '_filter_by_fba' );
    }
    ...
    public function replyToFilterProduct( $aProduct, $aRow, $aRowIdentifier ) {
        ...
        return ( boolean ) $_oRow->getCell( 'delivery_fba' )
            ? $aProduct
            : array();
